...
function interpolatePoints(left, right, amount) {
  return left.map((point, index) => interpolatePoint(point, right[index] || point, amount));
}

function sampleAssignmentLineIndices(assignments, limitPerCluster = 2) {
  if (!assignments) return [];
  const picked = [];
  const seen = new Map();
  assignments.forEach((cluster, index) => {
    if (cluster < 0) return;
    const count = seen.get(cluster) || 0;
    if (count < limitPerCluster) {
      picked.push(index);
      seen.set(cluster, count + 1);
    }
  });
  return picked;
}
...
function buildHeroKmeansFrames(points, k, seed, maxIterations = 3) {
  const rng = mulberry32(seed);
  const startCentroids = sampleUnique(points, k, rng);
  const frames = [
    {
      title: "Pick how many groups",
      assignments: null,
      ...
      ghostCentroids: [],
      showLines: false,
      labels: [{ screen: [0.12, 0.16], text: "no labels yet", color: "#ff8a62" }],
    },
    {
      title: "Random starting middles",
      assignments: null,
      centroids: [],
      initialCentroids: startCentroids.map(clonePoint),
      ghostCentroids: [],
      showLines: false,
      labels: [{ point: startCentroids[0], text: "random start", dx: 12, dy: -14, color: "#ff8a62" }],
    },
  ];

  let centroids = startCentroids.map(clonePoint);
  let assignments = Array(points.length).fill(-1);

  for (let loop = 0; loop < maxIterations; loop += 1) {
    const nextAssignments = points.map((point) => nearestCenterIndex(point, centroids));
    frames.push({
      title: loop === 0 ? "Dots join the nearest middle" : "Dots check again",
      assignments: nextAssignments.slice(),
      centroids: centroids.map(clonePoint),
      initialCentroids: [],
      ghostCentroids: [],
      showLines: true,
      labels: [
        { point: centroids[0], text: "group middle", dx: 12, dy: -16 },
        { screen: [0.62, 0.16], text: "nearest group", color: "#ff8a62" },
      ],
    });

    const nextCentroids = computeCentroids(points, nextAssignments, k, rng);
    frames.push({
      title: loop === 0 ? "Middles move to the average" : "Middles move again",
      assignments: nextAssignments.slice(),
      centroids: nextCentroids.map(clonePoint),
      initialCentroids: [],
      ghostCentroids: centroids.map(clonePoint),
      showLines: false,
      interpolate: true,
      labels: [
        { point: centroids[0], text: "old middle", dx: -58, dy: -16, color: "#ff8a62" },
        { point: nextCentroids[0], text: "new middle", dx: 12, dy: 18 },
      ],
    });
  ...
  frames.push({
    title: "Stop when nothing changes",
    assignments: assignments.slice(),
    centroids: centroids.map(clonePoint),
    initialCentroids: [],
    ghostCentroids: [],
    ...
    labels: [
      { point: centroids[0], text: "group middle", dx: 12, dy: -16 },
      { screen: [0.66, 0.16], text: "settled groups", color: "#ff8a62" },
    ],
  });
...
function describeScatterRegion(point) {
  const [x, y] = point;
  if (y > 1.35) return "top dots";
  if (x < -1.25) return "left-side dots";
  if (x > 1.25) return "right-side dots";
  if (y < -0.95) return "lower dots";
  return "middle dots";
}

function describeSpreadPlain(spread) {
  return spread < 0.85 ? "dots stay fairly close" : "dots spread wider";
}
...
function scatterSvg(options) {
  ...
  const assignmentLines =
    options.assignmentLineIndices && options.assignments && (options.centroids || []).length
      ? options.assignmentLineIndices
          .map((pointIndex) => {
            const cluster = options.assignments[pointIndex];
            const center = options.centroids[cluster];
            if (cluster < 0 || !center || !mappedPoints[pointIndex]) return "";
            const [x1, y1] = mappedPoints[pointIndex];
            const [x2, y2] = mapSinglePoint(center, bounds, width, height, padding);
...
const heroFrames = buildHeroKmeansFrames(data.kmeansMain, 3, 30);
...
function renderKmeansUnsupervised() {
  ...
  const markup = scatterSvg({
    ...
    note: state.kmeansReveal ? "near dots become one group" : "gray dots with no labels",
    ...
    annotations: state.kmeansReveal
      ? [
          ...
          { screen: [0.14, 0.16], text: "same kind", color: getCssVar("--accent-alt") },
          { screen: [0.68, 0.22], text: "grouped together" },
        ]
      : [{ screen: [0.12, 0.16], text: "no labels yet", color: getCssVar("--accent-alt") }],
  });
...
function renderAlgorithm() {
  ...
  const previousFrame = state.kmeansAlgorithm.step > 0 ? trace.frames[state.kmeansAlgorithm.step - 1] : null;
  ...
  refs["kmeans-algorithm-subtitle"].textContent =
    frame.step === 3
      ? "dots stay still; only group color and nearest-middle links change"
      : frame.step === 4
        ? "dots stay still while the group middles move to the average"
        : `step ${state.kmeansAlgorithm.step + 1} of ${trace.frames.length}`;
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      ghostCentroids: frame.step === 4 && previousFrame?.centroids?.length ? previousFrame.centroids : [],
      assignmentLineIndices: frame.step === 3 ? sampleAssignmentLineIndices(frame.assignments, 2) : [],
      note:
        frame.step === 0
          ...
          : frame.step === 1
            ...
            : frame.step === 2
              ? "the first middles appear"
              : frame.moved === 0
                ? "nothing is changing now"
                : "each dot moves to the nearest middle",
      annotations:
        frame.step === 0
          ? [{ screen: [0.12, 0.16], text: "pick K first", color: getCssVar("--accent-alt") }]
          : frame.step === 1
            ? [{ screen: [0.12, 0.16], text: "first guess", color: getCssVar("--accent-alt") }]
            : frame.step === 2
              ? [
                  { point: frame.centroids[0], text: "group middle", dx: 12, dy: -16 },
                  { screen: [0.58, 0.16], text: "guess the middles", color: getCssVar("--accent-alt") },
                ]
              : frame.step === 3
                ? [
                    { point: frame.centroids[0], text: "group middle", dx: 12, dy: -16 },
                    { screen: [0.58, 0.16], text: "nearest group", color: getCssVar("--accent-alt") },
                  ]
                : previousFrame?.centroids?.length
                  ? [
                      { point: previousFrame.centroids[0], text: "old middle", dx: -60, dy: -16, color: getCssVar("--accent-alt") },
                      { point: frame.centroids[0], text: "new middle", dx: 14, dy: 18 },
                    ]
                  : frame.centroids.length
                    ? [{ point: frame.centroids[0], text: "group middle", dx: 12, dy: -16 }]
                    : [],
    }),
  );
...
function renderChooseK() {
      ...
      annotations: [
        ...
        ...(state.kmeansSelectedK <= 2
          ...
          : state.kmeansSelectedK >= 5
            ...
            : [
                { screen: [0.42, 0.16], text: "about right", color: getCssVar("--accent-alt") },
                { screen: [0.64, 0.78], text: "good grouping" },
              ]),
      ],
    }),
  );
  ...
  setSvg(
    ...
    lineChartSvg(elbowValues, {
      ...
      title: "total squared distance (lower is better)",
  ...
  setSvg(
    ...
    lineChartSvg(silhouetteValues, {
      ...
      title: "average silhouette (higher is better)",
...
function renderRandomStarts() {
  ...
  refs["kmeans-random-summary"].textContent = `bad start versus best of ${cache.randomCases.length} tries`;
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      annotations: [{ point: current.initialCentroids[0], text: "bad start", dx: 12, dy: -14, color: getCssVar("--accent-alt") }],
    }),
  );
...
function renderScaleDemo() {
  ...
  const ranges = axisRanges(state.kmeansScaleMode === "raw" ? data.scaleData : cache.scaleInfo.scaled);

  refs["kmeans-scale-note"].textContent =
    state.kmeansScaleMode === "raw" ? "big numbers dominate" : "fairer comparison";
  refs["kmeans-scale-basis"].textContent = formatNumber(ranges.x, 2);
  refs["kmeans-scale-story"].textContent = formatNumber(ranges.y, 2);
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      note: state.kmeansScaleMode === "raw" ? "big numbers dominate" : "fairer comparison",
      annotations: [
        {
          screen: [0.62, 0.14],
          text: state.kmeansScaleMode === "raw" ? "big numbers dominate" : "fairer comparison",
        },
      ],
    }),
  );
...
function renderOutlierDemo() {
  ...
  refs["kmeans-outlier-inertia"].textContent = showOutlier
    ? formatNumber(withOutlierRun.inertia - baseRun.inertia, 2)
    : "0.00";
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      annotations: showOutlier
        ? [
            { point: baseRun.centroids[1], text: "old middle", dx: -58, dy: -16, color: getCssVar("--accent-alt") },
            { point: run.centroids[1], text: "new middle", dx: 12, dy: 18 },
            { screen: [0.54, 0.14], text: "group middle pulled away" },
          ]
...
function renderKmeansInterpretation() {
  ...
  const focusSummary = summaries[state.kmeansFocusCluster];
  const focusLabel = describeScatterRegion(focusSummary.centroid);
      ...
      return `
        ...
        >
          ...
          <div class="cluster-copy">
            ...
            <span>${describeScatterRegion(summary.centroid)}</span>
            <span>${describeSpreadPlain(summary.spread)}</span>
          </div>
        </button>
      `;
    })
...
function renderHierMerge() {
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      annotations: [
        { screen: [0.08, 0.8], text: "joins first" },
        { screen: [0.62, 0.2], text: "joins later", color: getCssVar("--accent-alt") },
      ],
    }),
  );
  ...
  setSvg(
    ...
    dendrogramSvg(cache.hierAverage, {
      ...
      labels: data.hierLabels,
      annotations: [
        { screen: [0.08, 0.84], text: "joins first" },
        { screen: [0.58, 0.26], text: "joins later", color: getCssVar("--accent-alt"), align: "middle" },
        { screen: [0.62, 0.14], text: "higher = less alike", align: "middle" },
      ],
    }),
  );
}
...
function renderHierLinkage() {
  ...
  const feelMap = {
    single: "one near pair can pull groups into a chain",
    average: "uses the mean distance across all pairs",
    complete: "every point must stay fairly close before joining",
  };
  ...
  refs["hier-linkage-warning"].textContent = feelMap[state.hierLinkage];
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      annotations: [{ screen: [0.08, 0.16], text: noteMap[state.hierLinkage], color: getCssVar("--accent-alt") }],
    }),
  );
...
function renderHierDistance() {
  ...
  refs["hier-distance-story"].textContent =
    state.hierDistance === "euclidean" ? "use this when amount matters more" : "use this when shape matters more";
  ...
  setSvg(
    ...
    dendrogramSvg(clustering, {
      ...
      annotations: [
        ...
        {
          screen: [0.62, 0.18],
          text: state.hierDistance === "euclidean" ? "how much there is" : "up-and-down shape",
          color: getCssVar("--accent-alt"),
          align: "middle",
        },
      ],
...
function renderHierScale() {
  ...
  const ranges = axisRanges(state.hierScaleMode === "raw" ? data.scaleData : cache.scaleInfo.scaled);

  refs["hier-scale-note"].textContent =
    state.hierScaleMode === "raw" ? "big numbers dominate" : "fairer tree";
  refs["hier-scale-basis"].textContent = formatNumber(ranges.x, 2);
  refs["hier-scale-story"].textContent = formatNumber(ranges.y, 2);
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      assignments,
      axes: { xLabel: "Socks", yLabel: "Computers" },
      note: state.hierScaleMode === "raw" ? "big numbers dominate" : "fairer comparison",
      annotations: [
        {
          screen: [0.54, 0.16],
          text: state.hierScaleMode === "raw" ? "big numbers dominate" : "fairer comparison",
          color: getCssVar("--accent-alt"),
        },
      ],
    }),
  ...
  setSvg(
    ...
    dendrogramSvg(clustering, {
      ...
      annotations: [
        {
          screen: [0.08, 0.16],
          text: state.hierScaleMode === "raw" ? "big numbers dominate" : "fairer tree",
        },
      ],
    }),
  );
...
function renderHierOutlier() {
  ...
  refs["hier-outlier-note"].textContent = showOutlier
    ...
    : "show the weird point to compare its last join";
  refs["hier-outlier-height"].textContent = showOutlier ? formatNumber(clustering.maxHeight, 2) : "-";
  refs["hier-outlier-story"].textContent = showOutlier ? "inspect that point" : "show it first";
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      outlierPoint: showOutlier ? data.outlierPoint : null,
      outlierText: "weird point",
      annotations: showOutlier ? [{ screen: [0.54, 0.16], text: "joins last", color: getCssVar("--accent-alt") }] : [],
    }),
  );
  setSvg(
    ...
    dendrogramSvg(clustering, {
      ...
      annotations: showOutlier
        ? [
            { screen: [0.48, 0.12], text: "lonely branch" },
            { screen: [0.72, 0.22], text: "joins last", color: getCssVar("--accent-alt") },
          ]
        : [],
    }),
  );
...
function renderHierFinal() {
  ...
  const groups = cutTree(cache.hierAverage.root, cutHeight);
  const orderedGroups = groups.slice().sort((left, right) => Math.min(...left) - Math.min(...right));
  const assignments = groupsToAssignments(orderedGroups, data.hierMain.length);
  state.hierFocusCluster = Math.min(state.hierFocusCluster, orderedGroups.length - 1);
  const width = 620;
  const height = 300;
  const layout = layoutDendrogram(cache.hierAverage.root, width, height, 28);
  const groupAnnotations = orderedGroups.map((group, index) => {
    const xs = group.map((member) => layout.positions.get(member).x);
    const averageX = xs.reduce((sum, x) => sum + x, 0) / xs.length;
    const center = meanPoint(group.map((member) => data.hierMain[member]));
    return {
      screen: [(averageX - layout.padding) / (width - layout.padding * 2), 0.86],
      text: describeScatterRegion(center),
      align: "middle",
      color: index === state.hierFocusCluster ? getCssVar("--accent-alt") : getCssVar("--accent"),
    };
  });
  ...
  setSvg(
    ...
    dendrogramSvg(cache.hierAverage, {
      ...
      cutLabel: "cut here",
      annotations: [{ screen: [0.74, 0.16], text: "read these groups", align: "middle" }, ...groupAnnotations],
    }),
  );

  refs["hier-final-cluster-cards"].innerHTML = orderedGroups
    .map((group, index) => {
      ...
      return `
        ...
        >
          ...
          <div class="cluster-copy">
            ...
            <span>${describeScatterRegion(center)}</span>
            <span>dots ${group.map((member) => member + 1).join(", ")}</span>
          </div>
        </button>
      `;
    })
...
function animateHeroPlots(time) {
  ...
  const total = heroFrames.length * frameDuration;
  const position = (time % total) / frameDuration;
  const frameIndex = Math.floor(position);
  const frame = heroFrames[frameIndex];
  const frameProgress = position - frameIndex;
  const centroids =
    frame.interpolate && frame.ghostCentroids.length
      ? interpolatePoints(frame.ghostCentroids, frame.centroids, frameProgress)
      : frame.centroids;
  ...
  setSvg(
    ...
    scatterSvg({
      ...
      assignments: frame.assignments,
      centroids,
      initialCentroids: frame.initialCentroids,
      ghostCentroids: frame.interpolate ? frame.ghostCentroids : [],
      assignmentLineIndices: frame.showLines ? sampleAssignmentLineIndices(frame.assignments, 2) : [],
      note: frame.title,
      legend: "plus sign = group middle",
      annotations: frame.labels,
    }),
  );
  ...
  setSvg(
    ...
    dendrogramSvg(cache.hierAverage, {
      ...
      annotations: [
        { screen: [0.08, 0.84], text: "small groups" },
        { screen: [0.54, 0.46], text: "bigger groups", align: "middle", color: getCssVar("--accent-alt") },
      ],
    }),
  );
